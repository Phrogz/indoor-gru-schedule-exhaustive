...
export class TreeReader {
	 ...
	 * @returns {Promise<{incompletePrefixes: number[][][], completedPrefixes: Set<string>, isPartial: boolean, lastIncompletePrefix: number[][] | null}>}
	 */
	async findIncompleteBranches() {
		...
		const incompleteMarkerOrder = []; // parent keys in marker encounter order
			...
			if (isIncompleteMarker(content) || isCompleteMarker(content) || content === '') {
				if (depth === leafDepth && currentParentKey !== null) {
					if (isCompleteMarker(content)) {
						...
						incompleteMarkerOrder.push(currentParentKey);
					}
				}
			} else {
		...
		let lastIncompletePrefix = null;
		for (let i = incompleteMarkerOrder.length - 1; i >= 0; i--) {
			const key = incompleteMarkerOrder[i];
			const state = prefixStates.get(key);
			if (state && !state.completeSeen && state.lastMarker === 'incomplete') {
				lastIncompletePrefix = key.split('|').map(s => s.split(',').map(v => parseInt(v, 10)));
				break;
			}
		}

		return { incompletePrefixes, completedPrefixes, isPartial, lastIncompletePrefix };
	}
}
