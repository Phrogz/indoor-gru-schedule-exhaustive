...
- **Week-0 files** (`results/{N}teams-week0.txt`): One schedule per line, comma-separated matchup indices
...
const rl = createInterface({ input: createReadStream('results/6teams-week0.txt') });
...
const schedules = readFileSync('results/6teams-week0.txt', 'utf8')
  .trim().split('\n')
...
|--------|---------|---------|
| `schedule-parallel.mjs` | Schedule enumeration (1+ weeks) | `node schedule-parallel.mjs --teams=6 --weeks=3` |
| `display-schedules.mjs` | View saved results | `node display-schedules.mjs results/8teams-2weeks.txt --limit 5` |
...
Core algorithm in both `schedule.mjs` and `schedule-parallel.mjs`:
...
2. **Week count**: `schedule-parallel.mjs` requires `N_WEEKS >= 2`
...
```bash
# Generate and save week-0 schedules (creates results/{N}teams-week0.js)
node schedule.mjs --teams=6

# Generate multi-week (loads week-0, saves {N}teams-{M}weeks.js)
node schedule-parallel.mjs --teams=8 --weeks=2

# Validate without file I/O
node schedule-parallel.mjs --teams=8 --weeks=2 --validate

# View results with grid display
node display-schedules.mjs results/8teams-2weeks.js --limit 3
```
